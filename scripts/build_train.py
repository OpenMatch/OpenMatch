...
def get_positive_and_negative_samples(
    query_dataset, corpus_dataset, qrel, n_sample, depth, q_and_docs
):
    qid = q_and_docs[0]
    ...
    if (len(origin_positives)) >= 1:
        item = process_one(
            query_dataset, corpus_dataset, qid, origin_positives, negatives[:n_sample]
        )
        if item["positives"]:
            return item
        else:
...
def process_one(query_dataset, corpus_dataset, q, poss, negs):
    train_example = {
        ...
        "positives": [
            corpus_dataset[p]["input_ids"] for p in poss if corpus_dataset[p]["input_ids"]
        ],
        "negatives": [
            corpus_dataset[n]["input_ids"] for n in negs if corpus_dataset[n]["input_ids"]
        ],
    }
