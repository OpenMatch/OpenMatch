...
def get_positive_and_negative_samples(
    query_dataset, corpus_dataset_pos, corpus_dataset_neg, qrel, n_sample, depth, q_and_docs
):
    qid = q_and_docs[0]
...
def process_one(query_dataset, corpus_dataset_pos, corpus_dataset_neg, q, poss, negs):
    train_example = {
        "query": query_dataset[q]["input_ids"],
        "positives": [
            corpus_dataset_pos[p]["input_ids"] for p in poss if corpus_dataset_pos[p]["input_ids"]
        ],
        "negatives": [
            corpus_dataset_neg[n]["input_ids"] for n in negs if corpus_dataset_neg[n]["input_ids"]
        ],
    }
...
qrel = (
    load_positives(other_args.qrels_file)
    if not other_args.beir
    else load_beir_positives(other_args.qrels_file)
)
run_list = []
