...
data_args.query_path = data_args.query_path or os.path.join(
    other_args.gpl_data_dir, "qgen-queries.jsonl"
)
data_args.corpus_path = data_args.corpus_path or os.path.join(
    other_args.gpl_data_dir, "corpus.jsonl"
)
...
with open(other_args.save_to, "w") as fout, open(
    os.path.join(other_args.gpl_data_dir, "gpl-training-data.tsv"), "r"
) as fin:
