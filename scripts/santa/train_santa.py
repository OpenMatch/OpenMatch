...
from trainer import STrainer, MappingDRTrainDataset
from transformers import AutoConfig, AutoTokenizer, HfArgumentParser, set_seed
from santa_arguments import SantaArguments
from model import SModel
from data_collator import QPCollator


logger = logging.getLogger(__name__)
def main():
    parser = HfArgumentParser((ModelArguments, DataArguments, TrainingArguments,SantaArguments))

    if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):
        model_args, data_args, training_args, santa_args = parser.parse_json_file(json_file=os.path.abspath(sys.argv[1]))
    else:
        model_args, data_args, training_args, santa_args = parser.parse_args_into_dataclasses()
        model_args: ModelArguments
        data_args: DataArguments
        training_args: TrainingArguments
        santa_args: SantaArguments
    ...
    train_dataset_cls = MappingDRTrainDataset
    ...
    trainer_cls = GCDenseTrainer if training_args.grad_cache else STrainer
    trainer = trainer_cls(
        model=model,
        args=training_args,
        tokenizer=tokenizer,
        ...
        data_collator=QPCollator(
            ...
            max_l_len= santa_args.l_max_len,
        ),
