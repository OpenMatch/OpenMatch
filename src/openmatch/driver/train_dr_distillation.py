...
def main():
    ...
    if training_args.distil_mode == "pairwise":
        train_dataset_cls = (
            ...
            else StreamPairwiseDistillationTrainDataset
        )
    elif training_args.distil_mode == "listwise":
        train_dataset_cls = (
            MappingListwiseDistillationTrainDataset
            if training_args.use_mapping_dataset
            else StreamListwiseDistillationTrainDataset
        )
    else:
        raise ValueError(f"Invalid distillation mode: {training_args.distil_mode}")
