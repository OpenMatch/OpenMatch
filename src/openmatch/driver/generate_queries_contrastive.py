...
from transformers import (AutoConfig, AutoTokenizer, HfArgumentParser, T5ForConditionalGeneration)
...
def main():
    ...
    if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):
        model_args, data_args, inference_args, other_args = parser.parse_json_file(json_file=os.path.abspath(sys.argv[1]))
    else:
    ...
    with open(inference_args.queries_save_path, "w") as f_queries, \
            open(inference_args.qrels_save_path, "w") as f_qrels:
        for ori_qid, pos_docid, neg_docid, queries in zip(ori_qids, pos_docids, neg_docids, generated_queries):
            for qid, q in enumerate(queries):
                if len(q) < 10:
                    continue
                f_queries.write(f"original_q_{ori_qid}_generated_q_{qid}_for_pos_{pos_docid}_neg_{neg_docid}\t{q}\n")
                f_qrels.write(f"original_q_{ori_qid}_generated_q_{qid}_for_pos_{pos_docid}_neg_{neg_docid}\t0\t{pos_docid}\t1\n")
                f_qrels.write(f"original_q_{ori_qid}_generated_q_{qid}_for_pos_{pos_docid}_neg_{neg_docid}\t0\t{neg_docid}\t0\n")
